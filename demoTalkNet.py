...
def crop_video(args, track, cropFile, fps):
    ...
    vOut = cv2.VideoWriter(
        cropFile + "t.avi", cv2.VideoWriter_fourcc(*"XVID"), fps, (224, 224)
    )  # Write video
    dets = {"x": [], "y": [], "s": []}
...
def evaluate_network(files, args, fps):
    ...
    for file in tqdm.tqdm(files, total=len(files)):
        ...
        audioFeature = python_speech_features.mfcc(
            audio, 16000, numcep=13, winlen=0.025, winstep=winstep
        )
        ...
        length = min(
            (audioFeature.shape[0] - audioFeature.shape[0] % 4) / (fps * 4),
            videoFeature.shape[0] / fps,
        )
            ...
            with torch.no_grad():
                for i in range(batchSize):
                    inputA = torch.FloatTensor(
                        audioFeature[
                            i * duration * (fps * 4) : (i + 1) * duration * (fps * 4), :
                        ]
                    ).unsqueeze(
                        0
                    ).cuda()
...
def get_source_fps(input_path):
    ...
    try:
        ...
        print(
            "something went wrong while reading FPS from video stream : {}".format(e),
        )
    return video_fps
...
def main():
    ...
    if (
        does_input_require_fps_manipulation
        and not does_input_require_resolution_manipulation
    ):
        ...
        does_input_require_resolution_manipulation
        and not does_input_require_fps_manipulation
    ):
        ...
        does_input_require_resolution_manipulation
        and does_input_require_fps_manipulation
    ):
        new_fps = get_new_fps(source_fps)
        new_width, new_height = get_new_width_height(metadata)
        new_video_name = "modified_input_video.mp4"
    ...
    if args.duration == 0:
        command = (
            "ffmpeg -y -i %s -qscale:v 2 -threads %d -async 1 -r %f %s -loglevel panic"
            % (
                source_video_path,
        ...
        command = (
            "ffmpeg -y -i %s -qscale:v 2 -threads %d -ss %.3f -to %.3f -async 1 -r %f %s -loglevel panic"
            % (
                source_video_path,
                args.nDataLoaderThread,
                args.start,
    ...
    for ii, track in tqdm.tqdm(enumerate(allTracks), total=len(allTracks)):
        vidTracks.append(
            crop_video(
                args, track, os.path.join(args.pycropPath, "%05d" % ii), source_fps
            )
        )
